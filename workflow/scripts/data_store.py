...
feature_offsets = r'/home/greydon/Documents/GitHub/Auto-afids/resources/feature_offsets.npz'
train_method = 'fine'
...
combined_files = dict(zip(nii_files, fcsv_files))
...
args = Namespace(iafid=1,output_dir=config['output_dir'], combined_files=combined_files, 
                 feature_offsets=feature_offsets, train_level='fine')
		...
		for iafid in range(1,33):
			...
			with open(ifcsv) as file:
				...
				if (hdr['qform_code'] > 0 and hdr['sform_code'] == 0):
					...
					fill = np.matmul(four,trying[iafid-1].reshape(-1,1))
					fill = fill.reshape(4)
					newarr.append(fill)
			...
			if not skip:
				...
				finalpredarr.append(np.asarray(finalpred))
		
		# Save to file
		data_out = os.path.join(args.output_dir, '{}_level-{}.npy'.format(os.path.basename(inii).split('_')[0], args.train_level))
		np.save(data_out, finalpredarr)
