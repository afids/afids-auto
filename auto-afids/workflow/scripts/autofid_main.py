...
def main(
    ...
    testing = (
        testing
        - 30
        + (test_voxels_fine_patch[idx] * (30 / 60.5) + (0.5 * (30 / 60.5)))
    )
...
def check_prediction_coarse(idx, test_voxels, img, smin, smax):
    ...
    print(f"check_prediction_coarse patch -- amin: {np.amin(patch)}, amax: {np.amax(patch)}")
    ...
    diff = process_testerarr(
        test_voxels_new, smin, smax, patch_cumsum, False
    )
...
def check_prediction_med(idx, test_voxels, img, smin, smax, testing):
    ...
    diff = process_testerarr(
        test_voxels_new, smin, smax, patch_cumsum, True
    )
