...
def imresizevec(inimg, weights, indices, dim):
    ...
    if dim == 0:
        ...
        outimg = np.sum(
            weights * ((inimg[:, :, indices].squeeze(axis=3)).astype(np.float64)),
            axis=3,
        )
    if inimg.dtype == np.uint8:
        outimg = np.clip(outimg, 0, 255)
...
def imresize(I, scalar_scale=None, method="bicubic", output_shape=None, mode="vec"):
    ...
    for dim in range(3):
        ...
        if dim == 0:
            weights = weights.reshape((wshape[0], wshape[2], 1, 1))
            B = np.sum(
                weights * ((B[indices].squeeze(axis=1)).astype(np.float64)), axis=1
            )
        elif dim == 1:
            weights = weights.reshape((1, wshape[0], wshape[2], 1))
            B = np.sum(
                weights * ((B[:, indices].squeeze(axis=2)).astype(np.float64)), axis=2
            )
        elif dim == 2:
            weights = weights.reshape((1, 1, wshape[0], wshape[2]))
            B = np.sum(
                weights * ((B[:, :, indices].squeeze(axis=3)).astype(np.float64)),
                axis=3,
            )
