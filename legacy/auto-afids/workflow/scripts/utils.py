...
def process_testerarr(voxels_to_test, smin, smax, img_cumsum, int_cornerlist):
    ...
    mincornerlist = np.zeros((4000 * voxels_to_test.shape[0], 3)).astype("uint8")
    maxcornerlist = np.zeros((4000 * voxels_to_test.shape[0], 3)).astype("uint8")

    # Fill the corner lists with each voxel plus smin/smax
    for index in range(voxels_to_test.shape[0]):
        mincornerlist[index * 4000 : (index + 1) * 4000] = voxels_to_test[index] + smin
        maxcornerlist[index * 4000 : (index + 1) * 4000] = voxels_to_test[index] + smax
    ...
    img_cumsum_expanded = np.zeros(
        (img_cumsum.shape[0] + 1, img_cumsum.shape[1] + 1, img_cumsum.shape[2] + 1)
    )
    img_cumsum_expanded[1:, 1:, 1:] = img_cumsum

    print(f"img_cumsum_expanded shape: {img_cumsum_expanded.shape}")
    print(f"All maxcorners: {cornerlist[:, 3]}, {cornerlist[:, 4]}, {cornerlist[:, 5]}")
    ...
    testerarr = (
        img_cumsum_expanded[
            cornerlist[:, 3] + 1, cornerlist[:, 4] + 1, cornerlist[:, 5] + 1
        ]
        - img_cumsum_expanded[
        ...
        + img_cumsum_expanded[cornerlist[:, 3] + 1, cornerlist[:, 1], cornerlist[:, 2]]
        + img_cumsum_expanded[cornerlist[:, 0], cornerlist[:, 1], cornerlist[:, 5] + 1]
        + img_cumsum_expanded[cornerlist[:, 0], cornerlist[:, 4] + 1, cornerlist[:, 2]]
        - img_cumsum_expanded[cornerlist[:, 0], cornerlist[:, 1], cornerlist[:, 2]]
    ) / (
