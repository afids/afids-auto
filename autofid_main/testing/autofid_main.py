...
# current = os.path.dirname(os.path.abspath('autofid_main.py'))
# os.chdir(current)
# image = current + '\\OAS1\\sub-0109_T1w.nii'
# ground_truth = current + '\\OAS1\\OAS1_0109_MR1_T1_MEAN.fcsv'
# fid = 1
...
def autofid_main(image, ground_truth=None, fid=1):
    ...
    if os.path.exists(imagefile[:-4]+'%s' % extra + '.npy') == True:
        ...
        np.save(image[:-4]+'%s' % extra + '.npy',diff_coarse)
